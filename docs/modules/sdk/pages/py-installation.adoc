...
|Base URL of the Maildeno API. Override only when proxying through your own infrastructure or running a self-hosted instance.
...
|`CacheConfig`
|memory
|Caching strategy and tuning (`type`, `path`, `ttl`, `max_entries`). Omit for memory caching with defaults. See xref:py-caching.adoc[Caching].
