...
All notable changes to Maildeno are documented here. Maildeno follows https://semver.org[Semantic Versioning]. The JavaScript/TypeScript and Python SDKs are versioned independently — the JS SDK is currently at **v2.1.3** and the Python SDK at **v2.0.3**.

== JavaScript / TypeScript SDK v2.1.3 — 2026-07-10
...
== JavaScript / TypeScript SDK v2.1.2 — 2026-06-21
...
== JavaScript / TypeScript SDK v2.1.1 — 2026-06-12

=== Fixed

* Visibility context now handles non-string values. Context passed as a number or boolean (e.g. `context: { premium: true }`, `context: { tier: 2 }`) was read as empty, so visibility rules comparing against it never matched and the affected rows were always hidden. Numbers and booleans are now coerced for comparison — `2` matches `"2"`, `true` matches `"true"`. String context was unaffected.
