...
Fetch a template's JSON definition and render it locally using the SDK's WASM engine. Merge tags and context never leave your server.
...
Returns the template's JSON definition — the `TemplateJson` payload the SDKs cache and render locally via the WASM engine. Treat the body as opaque unless you are building your own renderer against the exported `TemplateJson` type.
